...
def train(config):
    ...
    global_mean = np.load("global_mid_mel_feature_mean_per_feature.npy")
    global_std = np.load("global_mid_mel_feature_std_per_feature.npy")

    # Load dataset
    dataset = Explicit_Mid_Mel_PairDataset(
        root_dir="data/processed/mel_complement_features/mid/",
    ...
    torch.save(model.state_dict(), f"explicit_models/mid_mel_based/{name}.pt")
