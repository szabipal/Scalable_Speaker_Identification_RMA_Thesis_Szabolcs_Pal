...
def train(config):
    ...
    global_mean = np.load("global_exp_feature_mean_per_feature.npy")
    global_std = np.load("global_exp_feature_std_per_feature.npy")

    # Load dataset
    dataset = ExplicitPairDataset(
        root_dir="data/processed/mfcc_complement_features/",
    ...
    torch.save(model.state_dict(), f"explicit_models/mfcc_based/{name}.pt")
